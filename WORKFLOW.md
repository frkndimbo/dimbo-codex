...
## Codex Config Hygiene
- In `$HOME/.codex`, edit in-place; do not create `.bak`, `.backup.*`, `*.example`, or `*~` files.
- Git history is the rollback path for tracked policy/config files.
- Track only safe config/docs/scripts; never track `auth.json`, secret-bearing `config.toml`, sqlite DBs, logs, sessions, history, tmp/cache/vendor, or memory payload symlink targets.
- Stage specific files only; never `git add .` blindly.
- Delete transient task artifacts in the same session unless the user asks to keep them.
- Do not create files outside the active workspace unless explicitly directed.
- Prefer in-memory analysis over scratch files when output is transient.
- `logs_2.sqlite` may be purged when >100MB; `sessions/` older than 30 days may be pruned after explicit destructive confirmation; `.tmp/` plugin caches are ephemeral.
